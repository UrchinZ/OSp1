"""
RPI - 2017 Fall - Operating Systems(CSCI 4210) - Project 1 - Process Simulation Framework
Team Members - Xinyue Yan(yanx3), Judy Zhang (zhangz17)
"""
...
class System:
    ...
    def fcfsintegratedReady(self, time):
        if (self.numArrived == len(self.processes)):
            # no new processes come in
            self.fcfsBlock(time)
        else: 
            # still process 
            sortedp = sorted(self.processes, key = lambda x:(x.arrived, x.nextArrTime, x.id))
            tmpholder = []
            for i in sortedp:
                if i.arrived == 0 and i.nextArrTime <= time:
                    tmpholder.append(i)
            for i in self.block:
                if self.processes[self.findProcess(i)].nextArrTime <= time:
                    tmpholder.append(self.processes[self.findProcess(i)])
            tmpholder = sorted(tmpholder, key = lambda x: (x.nextArrTime, x.id))
            for i in tmpholder:
                if i.id in self.block:
                    self.fcfsBlock(i.nextArrTime)
                else:
                    self.fcfsReady(i.nextArrTime)
    ...
    def fcfsAlgo(self):
        ...
        while 1:
            ...
            if (self.ready):
                ...
                self.fcfsintegratedReady(self.cputime)
                ...
                self.fcfsintegratedReady(self.cputime-1)
            ...
            elif not self.ready and not self.block and self.numArrived != len(self.processes):
                # the arrival time is too far away all the process finished in the cpu, because not all, so first must be the nearest processes not being executed 
                unvisited = sorted(self.processes, key = lambda x:(x.arrived, x.nextArrTime, x.id))
                self.cputime = unvisited[0].nextArrTime
                self.fcfsReady(self.cputime)            
            else: 
    ...
    def rrBlock(self, time):
        ...
        for i in self.block:
            if (self.processes[self.findProcess(i)].nextArrTime <= time):
                ...
                # after finishing the io, print out statements
                self.block.remove(self.processes[self.findProcess(i)].id) # remove it from the io queue
                self.rrReady(self.processes[self.findProcess(i)].nextArrTime) 

    def rrBlockSort(self):
        # sort the block queue based on next arrival time
        sortblock = []
        for i in self.block:
            sortblock.append(self.processes[self.findProcess(i)])
    ...
    def rrAlgo(self):
        print("time {}ms: Simulator started for RR [Q {}]".format(0, '<empty>'))
        self.srtReady(self.cputime)
        while 1:
            self.rrBlock(self.cputime)
            self.rrReady(self.cputime)
            if self.ready:
                current_p = self.ready[0]
                current_p = self.findProcess(current_p)
                self.ready.remove(self.ready[0])
                ...
                self.cputime += self.t_cs/2 # switch in 
                self.numcswitch += 1 
                self.rrReady(self.cputime-1)
                self.rrBlock(self.cputime-1)
                # self.rrReady(self.cputime)
                # addup number of switch start
                self.run.append(self.processes[current_p].id)
                # add to running queue
                if self.processes[current_p].rmBurstTime == self.processes[current_p].CPUburstTime:
                    ...
                    print("time {}ms: Process {} started using the CPU with {}ms remaining [Q {}]".format(\
                        self.cputime, self.processes[current_p].id, self.processes[current_p].rmBurstTime, self.readyOut()))
                self.rrBlock(self.cputime)
                self.rrReady(self.cputime) # update ready queue 
                # flag to see if needs to preempt, preempt occurs if current process's remaining burst time > timeslice
                preempt = 0 
                # check io first see if there are any process that finishes io will preempt 
                if self.processes[current_p].rmBurstTime > self.tslice:
                    preempt = 1 # preemption will occur
                while preempt and self.processes[current_p].rmBurstTime - self.tslice > 0:
                    # check if there are any processes finish processing 
                    self.rrBlock(self.cputime + self.tslice)
                    self.rrReady(self.cputime + self.tslice)
                    self.cputime += self.tslice
                    self.processes[current_p].rmBstTimeSet(self.processes[current_p].rmBurstTime - self.tslice)
                    if self.ready:
                        # if the ready queue is not empty 
                        self.numpreemption += 1
                        print("time {}ms: Time slice expired; process {} preempted with {}ms to go [Q {}]".format(self.cputime, self.processes[current_p].id, self.processes[current_p].rmBurstTime, self.readyOut()))
                        self.processes[current_p].stopSet(self.cputime + self.t_cs/2)
                        self.ready.append(self.processes[current_p].id)
                        break
                    else:
                        print("time {}ms: Time slice expired; no preemption because ready queue is empty [Q {}]".format(self.cputime, self.readyOut()))
                        if self.processes[current_p].rmBurstTime > self.tslice:  # keep checking the tslice until the process terminating
                            preempt = 1
                        else: 
                            preempt = 0
                if preempt == 0:
                    self.rrBlock(self.cputime)
                    self.rrReady(self.cputime)
                    self.cputime += self.processes[current_p].rmBurstTime
                    self.rrBlock(self.cputime-1)
                    self.rrReady(self.cputime-1)
                    self.processes[current_p].exeBurst()
                    self.processes[current_p].addTadTime(self.cputime + self.t_cs/2 - self.processes[current_p].start)
                ...
                self.processes[current_p].nextArrTime = self.cputime + self.t_cs/2 + self.processes[current_p].iotime 
                # self.rrBlock(self.cputime)
                # self.rrReady(self.cputime)
                if self.processes[current_p].rmNumOfBurst > 0 and self.processes[current_p].iotime > 0 and preempt == 0:
                    print("time {}ms: Process {} switching out of CPU; will block on I/O until time {}ms [Q {}]".format(\
                        self.cputime, self.processes[current_p].id, self.processes[current_p].nextArrTime , self.readyOut()))
                    self.block.append(self.processes[current_p].id)
                self.rrBlock(self.cputime)
                ...
                # the arrival time is too far away all the process finished in the cpu 
                unvisited = sorted(self.processes, key = lambda x:(x.arrived, x.nextArrTime, x.id))
                self.cputime = unvisited[0].nextArrTime
                self.rrReady(self.cputime)
            else: 
                print("time {}ms: Simulator ended for RR".format(self.cputime))
                break
